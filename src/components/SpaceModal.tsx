...
export function SpaceModal({ space, multiplier, hasShield, onDrink, onUseShield, onClose }: Props) {
  useEffect(() => {
  ...
  const drinkCount = isSpecialRule ? null : (space.drinks as number) * multiplier;

  function handleDrink() {
    if (drinkCount !== null && drinkCount > 0) onDrink(drinkCount);
    onClose();
  }
  function handleShield() { onUseShield(); onClose(); }
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 md:inset-0 md:flex md:items-center md:justify-center md:p-4">
        <div className="bg-gray-800 rounded-t-3xl md:rounded-3xl w-full md:max-w-md shadow-2xl"
          ...
          <div className="flex justify-end px-4 pt-2">
            <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-700 text-gray-400 hover:text-white text-lg">×</button>
          </div>

          <div className="px-5 pb-6 space-y-4">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center shrink-0" style={{ backgroundColor: `${space.color}30` }}>
            ...
            <div className="rounded-2xl p-4" style={{ backgroundColor: `${space.color}20`, border: `1px solid ${space.color}50` }}>
              <div className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: space.color }}>Regra do Drinking Game</div>
              ...
              {!isSpecialRule && drinkCount !== null && drinkCount > 0 && (
                <div className="mt-3 flex items-center gap-2">
                  ...
                  {multiplier > 1 && (
                    <span className="text-xs text-red-400 bg-red-900/40 px-1.5 py-0.5 rounded-full font-bold">{multiplier}x</span>
                  )}
                </div>
              )}
              ...
              {isSpecialRule && space.id === 'vs' && (
                <div className="mt-2 text-sm text-gray-300">Segue a seção de minigames abaixo.</div>
              )}
            </div>
